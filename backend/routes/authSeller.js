...
// ACTION : REGISTER
router.post("/signup-seller", (req, res, next) => {
  const {
    mail,
  ...
  userModel
    .create(newUser)
    .then(dbRes =>
      res.status(200).json({ msg: "OK: seller created ! :) ", dbRes })
    )
    .catch(dbErr =>
      res.status(500).json({ msg: "FAIL: seller not created! :( ", dbErr })
    );
  // .catch(next);
});
...
router.post("/signin", (req, res, next) => {
  passport.authenticate("local", (err, user, failureDetails) => {
    if (err || !user) return res.status(403).json("invalid seller infos"); // 403 : Forbidden
...
router.post("/signout-seller", (req, res, next) => {
